...
public class AiController {
    ...
    public AiController(GradeService gradeService, AiService aiService, AttendanceService attendanceService,
    ...
    public ResponseEntity<?> predict(@PathVariable Long studentId) {
        ...
        request.put("attendance", attendance.isEmpty() ? 0 : attendance.stream().mapToDouble(Double::doubleValue).average().orElse(0));
        request.put("assignmentMarks", scores);
        request.put("previousGpa", gradeService.calculateGpaSummary(studentId).get("gpa"));
        request.put("submissionDelay", submissions.stream().filter(s -> "LATE".equalsIgnoreCase(s.getStatus())).count());

        Map<String, Object> result = callAiService(request);

        AiPrediction p = new AiPrediction();
        p.setStudentId(studentId);
        p.setPredictedScore(Double.parseDouble(result.getOrDefault("predictedScore", result.get("prediction")).toString()));
    ...
    public ResponseEntity<?> insights(@PathVariable Long studentId) {
        ...
        long missingAssignments = submissionService.getSubmissionsByStudentId(studentId).stream()
                .filter(s -> "MISSING".equalsIgnoreCase(s.getStatus()) || "LATE".equalsIgnoreCase(s.getStatus()))
                .count();

        if (avgAttendance > 0 && avgAttendance < 75) insights.add("Attendance below safe level.");
        if (avgMarks > 0 && avgMarks < 60) insights.add("Low marks detected. Review weak subjects before the next assessment.");
        if (missingAssignments > 0) insights.add("Assignment submission rate is low.");
        if (insights.isEmpty()) insights.add("Performance indicators are currently stable.");
    ...
    private Map<String, Object> callAiService(Map<String, Object> request) {
        try {
            ...
            ResponseEntity<Map> response = restTemplate.postForEntity("http://localhost:8000/predict", entity, Map.class);
        ...
        double avg = previousMarks.stream().mapToDouble(Double::doubleValue).average().orElse(0);
        ...
        double predicted = Math.min(100, Math.max(0, (avg * 0.55) + (attendance * 0.25) + (gpa * 10 * 0.2)));
        String risk = predicted < 60 || attendance < 75 ? "High" : predicted < 75 ? "Medium" : "Low";
        String suggestion = attendance < 75
                ? "Attendance below safe level. Attend more classes and revise missed topics."
                : predicted < 70 ? "Improve preparation in low-scoring subjects and submit assignments on time."
                : "Maintain consistency and keep assignment submissions current.";
        return Map.of("studentId", request.get("studentId"), "predictedScore", predicted, "risk", risk, "suggestion", suggestion);
    }
